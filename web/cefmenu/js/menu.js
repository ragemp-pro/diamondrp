...
class MenuItem {
	 ...
	 */
	MoveUP(callback) {
		this.onmoveup = callback;

		return this;
	}
	MoveDOWN(callback) {
		this.onmovedown = callback;
...
Vue.component('popup-menu', {
	...
	methods: {
		processMoveUP: function(index) {
			if ((typeof this.currentMenu.currentItem().onmoveup) == 'function') {
				this.currentMenu.currentItem().onmoveup(index);
			}
		},
		processMoveDown: function(index) {
			if ((typeof this.currentMenu.currentItem().onmovedown) == 'function') {
				this.currentMenu.currentItem().onmovedown(index);
			}
		},
	...
	created: function() {
		...
		window.addEventListener('keyup', function(e) {
			...
			if (e.keyCode == KeyCode.Up) {
				...
				//window.mp.trigger("menuDestroy");
				window.closeMenu(true);
			} else if (e.keyCode == KeyCode.PUp) {
				self.processMoveUP(self.currentMenu.index);
			} else if (e.keyCode == KeyCode.PDown) {
				self.processMoveDown(self.currentMenu.index);
			}
